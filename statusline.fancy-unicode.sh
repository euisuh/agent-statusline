...
if [ -n "$CAVEMAN_MODE" ]; then
  if [ "$CAVEMAN_MODE" = "FULL" ]; then
    LINE1+="${BOLD}${YELLOW}[CAVEMAN]${RESET}"
  else
    LINE1+="${BOLD}${YELLOW}[CAVEMAN:${CAVEMAN_MODE}]${RESET}"
  fi
  LINE1+="$SEP"
fi
LINE1+="${BOLD}${MAGENTA}✦ ${MODEL_SHORT}${RESET}"
LINE1+="$SEP"
LINE1+="${CTX_COLOR}${BOLD}${BAR_FILLED}${BAR_EMPTY}${RESET} ${CTX_COLOR}${CTX_PCT}%${RESET}${GRAY}/${CTX_SIZE_FMT}${RESET}"
LINE1+="$SEP"
...
if $IS_GIT; then
  ...
  if [ "$GIT_AHEAD" -gt 0 ] && [ "$GIT_BEHIND" -gt 0 ]; then
    LINE2+=" ${YELLOW}↑${GIT_AHEAD}↓${GIT_BEHIND}${RESET}"
  ...
  GIT_PARTS=""
  [ "$GIT_STAGED" -gt 0 ]   && GIT_PARTS+="${GREEN}●${GIT_STAGED}staged${RESET} "
  [ "$GIT_UNSTAGED" -gt 0 ] && GIT_PARTS+="${YELLOW}✎${GIT_UNSTAGED}changed${RESET} "
  [ "$GIT_UNTRACKED" -gt 0 ]&& GIT_PARTS+="${GRAY}?${GIT_UNTRACKED}new${RESET} "

  if [ -n "$GIT_PARTS" ]; then
    LINE2+="$GIT_PARTS"
  else
    LINE2+="${GREEN}✔ clean${RESET}"
  fi
else
...
if [ -n "$RL5H_PCT" ]; then
  ...
  RL5H_RESET_FMT=""
  [ -n "$RL5H_RESET" ] && RL5H_RESET_FMT=" ${GRAY}resets ${RESET}${CYAN}$(fmt_reset "$RL5H_RESET")${RESET}"
...
if [ -n "$RL7D_PCT" ]; then
  ...
  RL7D_RESET_FMT=""
  [ -n "$RL7D_RESET" ] && RL7D_RESET_FMT=" ${GRAY}resets ${RESET}${CYAN}$(fmt_reset "$RL7D_RESET")${RESET}"
  [ -n "$LINE3" ] && LINE3+="$SEP"
...
# ✨ = 2 cells, 🧠 = 2 cells — both get 1 trailing space like line 2's folder icon
[ -n "$LINE3" ] && LINE3="${BOLD}${MAGENTA}✨${RESET} ${LINE3}"
...
if [ -n "$CODEX_QUOTA" ] && echo "$CODEX_QUOTA" | python3 -c "import sys,json; d=json.load(sys.stdin); exit(0 if 'primary_pct' in d else 1)" 2>/dev/null; then
  ...
  if [ "$CX_LIM" = "True" ]; then
    [ "$(pct_int "$CX_PRI")" -ge 100 ] && CX_PRI=100
    [ "$(pct_int "$CX_SEC")" -ge 100 ] && CX_SEC=100
  fi
  ...
  LINE4="🧠 ${GRAY}5h ${RESET}${CX_PRI_COLOR}${CX_PRI_BAR} $(fmt_pct $CX_PRI)${RESET} ${GRAY}resets ${RESET}${CYAN}${CX_PRI_RST}${RESET}"
  LINE4+="$SEP"
  LINE4+="${GRAY}7d ${RESET}${CX_SEC_COLOR}${CX_SEC_BAR} $(fmt_pct $CX_SEC)${RESET} ${GRAY}resets ${RESET}${CYAN}${CX_SEC_RST}${RESET}"
fi
